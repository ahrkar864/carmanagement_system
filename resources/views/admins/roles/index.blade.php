                        ...
                        @foreach($roles as $key => $role)
                            <tr class="shadow-lg rounded-full ">
                                ...
                                <td class="border-r-2 rounded-lg border-gray-300 bg-gray-50 bg-opacity-75 py-1.5 p-4 text-left text-sm text-gray-900 backdrop-blur backdrop-filter sm:pl-6 lg:pl-8">
                                    <div class="flex">
                                    <a data-id="{{ $role->id }}" class="editModal inline-flex items-center justify-center rounded-md border border-transparent bg-yellow-50 p-2 text-sm font-bold shadow-md hover:bg-yellow-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 sm:w-auto mr-2" >
                                    ...
                                    <a data-id="{{ $role->id }}" data-url="{{ route('roles.destroy', $role->id) }}" class="deleteModal inline-flex items-center justify-center rounded-md border border-transparent bg-rose-50 p-2 text-sm font-bold shadow-md hover:bg-rose-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2 sm:w-auto mr-2">
...
<div class="hidden relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true" id="roleModal"
                ...
                <div class="card border border-cyan-700 rounded ">
                    <div class="card-header border-b-2">
                        <div class="flex px-4 py-2 justify-between items-center">
                            <h3 class="font-bold text-cyan-600">Create Role</h3>
                           <button class="text-rose-600 font-extrabold"  onclick="closeModal('#roleModal')">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            ...
                            <div class="mt-2 grid grid-cols-4 gap-4 sm:grid-cols-6">
                                ...
                                <div class="col-span-4 sm:col-span-6">
                                    <label for="role_name" class="block text-sm font-medium text-gray-700">
                                        Name <span class="required_field">*</span>
                                    </label>
                                ...
                                <div class="col-span-6">
                                    <label for="permission" class="block text-sm font-medium text-gray-700">Check Permissions</label>
                                    <div class="mt-1 grid grid-cols-4">
                                        @foreach($permissions as $permission)
                                            <div class="relative flex items-start col-span-1 mx-1">
                                                <div class="flex h-5 items-center">
                                                    <input aria-describedby="permission-description" name="permissions[]" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-cyan-600 focus:ring-cyan-500" value="{{ $permission->id }}" id="permission_{{ $permission->id }}">
                                                </div>
                                    ...
                                    <span class="text-rose-600 error-text" id="permissionsError" style="font-size: 13px"></span>
                                </div>
...
<script src="{{ asset('/js/role_crud.js') }}"></script>
